...
const Skills = () => {
  const skillCategories = [
    ...
    },
    {
      ...
      icon: <Wrench className="text-green-400" size={28} />,
      skills: [
  ...
  return (
    <section id="skills" className="py-20 px-6 relative">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-40 left-10 w-40 h-40 bg-blue-500/10 rounded-full blur-2xl"></div>
        <div className="absolute bottom-40 right-10 w-40 h-40 bg-purple-500/10 rounded-full blur-2xl"></div>
      </div>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill, skillIndex) => (
                    <span
                      key={skillIndex}
                      className="px-3 py-2 text-sm glass text-muted-foreground rounded-full hover:bg-blue-500/20 hover:text-blue-400 transition-all duration-200 cursor-default hover:scale-105"
                    >
                      {skill}
                    </span>
                  ))}
